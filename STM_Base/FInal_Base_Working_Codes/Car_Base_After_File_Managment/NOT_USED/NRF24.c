...
#include "NRF24.h"
#include "main.h"   // ← add this line, gives access to Error_Handler


// change the number to the corresponding used ones
...
void nrf24_defaults(void){
    ...
    for(uint8_t i = 0; i < 6; i++){   // ← was 5, misses pipe 5
        nrf24_pipe_pld_size(i, 0);
        nrf24_cls_rx_pipe(i);
        nrf24_set_rx_dpl(i, disable);
        nrf24_auto_ack(i, disable);    // ← was enable, conflicts with no-ACK setup
    }

    // NOTE: ce_high() removed — defaults should leave CE low
}
...
// nrf24_cold_start() deleted — use NRF24_ColdStart() in main.c instead
